...
async function loadUrl(url) {
  try {
    if (url === "posts/") url = "asdf"; // force 404 if url is empty
    const page = await fetch(url);
    if (page.status !== 200) throw new Error();
...
window.addEventListener("popstate", (event) => {
  if (event.target.location.hash) loadUrl(event.target.location.hash.slice(1));
  else loadLatest();
});
