...
# replace LAST_POST_DATE with last post filename in index_dev.js
sed $(echo "${SEDCOMMAND}") index_dev.html > index.html &&

# replace LAST_POST_DATE with last post filename in index_tmp.js
sed $(echo "${SEDCOMMAND}") index_dev.js > index_tmp.js &&
